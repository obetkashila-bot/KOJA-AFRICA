...
CREATE TABLE IF NOT EXISTS public.cv_records (
 ...
 profile text, education text, experience text, skills text, professional_references text, file_name text, file_path text, file_url text,
 created_at timestamptz DEFAULT now(), updated_at timestamptz DEFAULT now()
);
...
-- ============================================================
-- KOJA AFRICA COMPLETE SECURITY / NOTIFICATIONS MIGRATION
-- ============================================================
...
create index if not exists email_tokens_expiry_idx on public.email_tokens(expires_at);
...
create index if not exists app_errors_resolved_idx on public.app_errors(resolved, created_at desc);
