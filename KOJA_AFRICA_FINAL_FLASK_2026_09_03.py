...
def create_delivery_request():
    ...
    payload={
        ...
        "pickup_address":clean(body.get("pickup_address")) or clean(body.get("pickup_location")),
        "destination":clean(body.get("destination")),
        "delivery_address":clean(body.get("delivery_address")) or clean(body.get("destination")),
        "pickup_latitude":lat,
    ...
    if error:
        minimal={
            ...
            "pickup_address":payload.get("pickup_address") or payload["pickup_location"],
            "destination":payload["destination"],
            "delivery_address":payload.get("delivery_address") or payload["destination"],
...
def deliveries():
    ...
    if request.method=="POST":
        ...
        tracking=make_tracking_code()
        pickup=clean(request.form.get("pickup_location"))
        destination=clean(request.form.get("destination"))
        payload={
            "id":str(uuid.uuid4()),"customer_id":user["id"],"user_id":user["id"],"sender_id":user["id"],
            "pickup_location":pickup,"pickup_address":pickup,
            "destination":destination,"delivery_address":destination,
