...
def research_ai_notes(query, results, style='apa'):
    """Generate structured study notes from retrieved evidence, with source-number citations."""
    if not results: return ''
    api_key=os.getenv('AI_API_KEY') or os.getenv('OPENAI_API_KEY')
    endpoint=os.getenv('AI_API_URL','https://api.openai.com/v1/chat/completions'); model=os.getenv('AI_MODEL','gpt-4o-mini')
    source_text='\n\n'.join(f"[{i+1}] {r.get('title','')} | {r.get('source','')} | {r.get('year') or 'n.d.'}\nAuthors: {', '.join(_names(r)) or 'Unknown'}\nEvidence: {r.get('snippet','')[:1600]}\nURL: {r.get('url','')}" for i,r in enumerate(results[:12]))
    prompt=f'''Create clear academic research notes for this question: {query}

Use ONLY the supplied source evidence. Do not invent facts, statistics, authors, dates, quotations, or references. Organize the notes as:
Title
Introduction
1. Key point
2. Key point
3. Key point
4. Key point (when supported)
Discussion / synthesis
Conclusion
Research gaps or limitations (only if supported)
References

Write complete paragraphs, not disconnected bullet fragments. Put source-number citations such as [1] and [2] immediately after claims they support. Do not cite a source for a claim it does not support. The selected bibliography style is {CITATION_STYLES.get(style, style)}. Keep the writing original and useful for a student.

SUPPLIED SOURCES:
{source_text}'''
    if api_key:
        try:
            payload={'model':model,'temperature':0.2,'max_tokens':1800,'messages':[{'role':'system','content':'You are KOJA Research Notes. Produce evidence-grounded academic notes. Never fabricate evidence. Cite supplied sources by number.'},{'role':'user','content':prompt}]}
            ...
            if rr.ok:
                ...
                if text: return text
        except Exception as exc: logger.warning('AI research notes failed: %s',exc)
    lines=[f'Title: {query}','','Introduction','','The following notes are based on the retrieved source evidence. Verify important claims against the original sources.','']
    for i,r in enumerate(results[:6],1):
        snippet=clean(r.get('snippet') or '')
        if snippet: lines += [f'{i}. {clean(r.get("title") or "Source")}','',f'{snippet} [{i}]','']
    lines += ['Conclusion','','The retrieved evidence provides a starting point for further research. Compare the original sources before using these notes in assessed academic work.']
    return '\n'.join(lines)
...
def research_notes():
    q=clean(request.values.get('q','')); style=clean(request.values.get('style','apa')).lower() or 'apa'
    if style not in CITATION_STYLES: style='apa'
    results=[]
    if q:
        results += research_web(q,8)+research_wikipedia(q,6)+research_openalex(q,None,8)+research_crossref(q,None,'',8)+research_local_documents(q,12)
        results=_research_filter(results,'all','', 'relevance')[:12]
    notes=research_ai_notes(q,results,style) if q else ''
    bibliography=make_bibliography(results,style) if results else []
    return render_page('Research Notes', r'''<div class="research-shell"><div class="hero"><h2>📝 KOJA Research Notes</h2><p>Turn retrieved evidence into clear, organized academic notes with citations and references.</p><form method="get" action="{{ url_for('research_notes') }}" class="research-search" style="margin-top:18px"><input name="q" value="{{ q }}" placeholder="Enter a research topic or question…" required><select name="style" aria-label="Citation style">{% for k,v in citation_styles.items() %}<option value="{{k}}" {% if style==k %}selected{% endif %}>{{v}}</option>{% endfor %}</select><button class="btn" type="submit">Write Notes</button></form></div>{% if q %}<div class="card"><strong>Topic:</strong> {{ q }}<br><span class="small">{{ results|length }} evidence sources retrieved. Verify important claims against original sources.</span></div>{% if notes %}<div class="card research-notes"><h2>Research Notes</h2><div style="white-space:pre-wrap;line-height:1.75">{{ notes }}</div></div>{% endif %}{% if bibliography %}<div class="card"><h2>References — {{ citation_styles[style] }}</h2>{% for n,ref in bibliography %}<p style="padding-left:28px;text-indent:-28px;line-height:1.6">{{ ref|safe }}</p>{% endfor %}</div>{% endif %}{% else %}<div class="grid"><div class="card"><h3>1. Ask</h3><p>Enter a research question or topic.</p></div><div class="card"><h3>2. Research</h3><p>KOJA retrieves web, scholarly and KOJA document evidence.</p></div><div class="card"><h3>3. Write</h3><p>KOJA organizes evidence into introduction, key points, discussion and conclusion.</p></div><div class="card"><h3>4. Cite</h3><p>Choose a citation style and generate references.</p></div></div>{% endif %}</div>''',q=q,results=results,notes=notes,style=style,citation_styles=CITATION_STYLES,bibliography=bibliography)
