...
-- V2 additive workflow upgrade
alter table if exists public.koja_professional_transactions add column if not exists appointment_id uuid;
create index if not exists koja_ps_tx_appointment_idx on public.koja_professional_transactions(appointment_id);
alter table if exists public.koja_professional_live_sessions add column if not exists service_id uuid;
create index if not exists koja_ps_live_service_idx on public.koja_professional_live_sessions(service_id,scheduled_at desc);
create table if not exists public.koja_professional_job_milestones (
 id uuid primary key default gen_random_uuid(), job_id uuid not null, position integer not null default 1,
 title text not null, description text, amount numeric(14,2) default 0, currency text default 'ZMW',
 status text default 'pending', created_at timestamptz default now(), updated_at timestamptz default now()
);
create index if not exists koja_ps_milestones_job_idx on public.koja_professional_job_milestones(job_id,position);
