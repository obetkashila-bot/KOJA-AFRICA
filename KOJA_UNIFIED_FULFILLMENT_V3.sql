...
create table if not exists public.koja_fulfillment_ai_events (
 ...
 order_id uuid, delivery_id uuid, input_summary text, recommendation text,
 provider text, status text not null default 'completed', created_at timestamptz not null default now()
);
create index if not exists koja_fulfillment_ai_events_user_idx on public.koja_fulfillment_ai_events(user_id,created_at desc);
create index if not exists koja_fulfillment_ai_events_delivery_idx on public.koja_fulfillment_ai_events(delivery_id,created_at desc);
