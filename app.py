...
def connect():
    ...
    return render_page('KOJA Connect',r'''<div class="hero"><h2>💬 KOJA Connect</h2><p>Chat, voice messages, voice calls, video calls, photos, files, groups and status updates with other KOJA users.</p></div><div class="card"><h3>⚡ Communication Buttons</h3><div class="grid"><a class="btn" href="{{ url_for('connect_people') }}">👥 Find / Connect People</a><a class="btn" href="{{ url_for('connect_status') }}">🟢 Post 24-hour Status</a><a class="btn" href="{{ url_for('connect_calls') }}">📞 Call History / Answer</a><button class="btn secondary" type="button" onclick="alert('Open a chat to use files and voice messages.')">📎 Files / Images / Voice</button></div></div><div class="card"><h3>Recent Chats</h3>{% for c in conversations %}<a class="card" style="display:block;text-decoration:none;color:inherit" href="{{ url_for('connect_chat',conversation_id=c.id) }}"><strong>{{ c._other_name }}</strong><div class="small">{{ c._last }}</div></a>{% else %}<p>No chats yet. Find a KOJA user to start.</p>{% endfor %}</div>''',conversations=conversations)
