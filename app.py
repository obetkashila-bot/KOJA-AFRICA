...
// Instant public Like: optimistic UI first, server confirmation in background.
document.addEventListener('DOMContentLoaded',function(){
  document.querySelectorAll('.like-form').forEach(function(form){
    if(form.dataset.likeReady==='1')return; form.dataset.likeReady='1';
    form.addEventListener('submit',function(ev){
      ev.preventDefault();
      var btn=form.querySelector('.like-btn'), count=form.querySelector('.like-count'); if(!btn||form.dataset.busy==='1')return;
      var wasLiked=btn.classList.contains('liked'); var oldCount=parseInt(count.textContent||'0',10)||0;
      var newLiked=!wasLiked; var newCount=Math.max(0,oldCount+(newLiked?1:-1));
      btn.classList.toggle('liked',newLiked); btn.firstChild.textContent=newLiked?'❤️ Liked':'🤍 Like'; count.textContent=newCount; btn.classList.add('like-flash'); setTimeout(function(){btn.classList.remove('like-flash')},240);
      form.dataset.busy='1'; btn.classList.add('like-busy');
      fetch(form.action,{method:'POST',headers:{'X-Requested-With':'XMLHttpRequest','Accept':'application/json'},credentials:'same-origin',keepalive:true})
      .then(function(r){if(!r.ok)throw new Error('like failed');return r.json()})
      .then(function(d){if(!d.ok)throw new Error('like failed'); btn.classList.toggle('liked',!!d.liked); btn.firstChild.textContent=d.liked?'❤️ Liked':'🤍 Like'; count.textContent=d.like_count})
      .catch(function(){btn.classList.toggle('liked',wasLiked);btn.firstChild.textContent=wasLiked?'❤️ Liked':'🤍 Like';count.textContent=oldCount})
      .finally(function(){form.dataset.busy='0';btn.classList.remove('like-busy')});
    });
  });
});
...
def public_toggle_like(post_id):
    ...
    likes=db_select('koja_public_likes', {'post_id':post_id}, select='user_id', limit=10000) or []
    liked=any(str(x.get('user_id'))==str(uid) for x in likes)
    next_path=request.args.get('next') if request.args.get('next') in ['/public','/news'] else '/public'
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or 'application/json' in (request.headers.get('Accept') or ''):
        return jsonify({'ok':True,'liked':liked,'like_count':len(likes),'post_id':post_id})
