...
def connect_group_call(call_id):
    ...
    return render_page('KOJA Group Video',r'''<div class="card"><a href="{{ url_for('connect_chat',conversation_id=call.conversation_id) }}">← Group Chat</a><h2>🎥 {{ c.get('name') or 'KOJA Group' }} — Group Video</h2><div id="state">Joining group video…</div></div><div id="grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px"></div><div class="card"><button id="mute" class="btn secondary">🎙️ Mute</button><button id="camera" class="btn secondary">📷 Camera</button><button id="hang" class="btn danger">End / Leave</button></div><script>
const callId={{ call.id|tojson }},ME={{ uid|tojson }},CSRF={{ csrf_token|tojson }},grid=document.getElementById('grid'),state=document.getElementById('state');let localStream=null,peers={},lastSignal='';
const api=async(u,o={})=>{o.cache='no-store';o.headers=Object.assign({},o.headers||{});if(o.method&&o.method.toUpperCase()!=='GET')o.headers['X-KOJA-CSRF']=CSRF;let r=await fetch(u,o),d={};try{d=await r.json()}catch(e){}if(!r.ok)throw Error(d.error||('HTTP '+r.status));return d;};
function addTile(id,name,stream,local=false){let old=document.getElementById('tile-'+id);if(old){old.querySelector('video').srcObject=stream;return;}let wrap=document.createElement('div');wrap.id='tile-'+id;wrap.className='card';wrap.innerHTML='<div style="font-weight:700;margin-bottom:5px">'+esc(name)+'</div><video autoplay playsinline '+(local?'muted':'')+' style="width:100%;aspect-ratio:4/3;object-fit:cover;background:#111;border-radius:10px"></video>';grid.appendChild(wrap);wrap.querySelector('video').srcObject=stream;}
function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
async function send(to,type,payload){await api('/api/connect/group-call/signal/'+encodeURIComponent(callId),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({recipient_id:to,signal_type:type,payload:payload})});}
async function makePeer(id,name,offerer){if(peers[id])return peers[id];let pc=new RTCPeerConnection({iceServers:{{ ice_servers|tojson }}});peers[id]={pc,name};localStream.getTracks().forEach(t=>pc.addTrack(t,localStream));pc.ontrack=e=>{if(e.streams&&e.streams[0])addTile(id,name,e.streams[0],false);};pc.onicecandidate=e=>{if(e.candidate)send(id,'ice',e.candidate.toJSON()).catch(()=>{});};pc.onconnectionstatechange=()=>{if(['failed','closed','disconnected'].includes(pc.connectionState)){setTimeout(()=>{if(peers[id]){try{peers[id].pc.close()}catch(e){}delete peers[id];}},1500);}};if(offerer){let o=await pc.createOffer();await pc.setLocalDescription(o);await send(id,'offer',{sdp:o.sdp});}return peers[id];}
async function process(sig){let from=String(sig.sender_id),name='KOJA User';let p=peers[from];if(sig.signal_type==='offer'){if(!p)p=await makePeer(from,name,false);await p.pc.setRemoteDescription(sig.payload);let a=await p.pc.createAnswer();await p.pc.setLocalDescription(a);await send(from,'answer',{sdp:a.sdp});}else if(sig.signal_type==='answer'){if(p)await p.pc.setRemoteDescription(sig.payload);}else if(sig.signal_type==='ice'){if(p)try{await p.pc.addIceCandidate(sig.payload)}catch(e){}}}
async function refresh(){try{let d=await api('/api/connect/group-call/active/'+encodeURIComponent({{ call.conversation_id|tojson }}));if(!d.call||d.call.id!==callId){state.textContent='Group call ended.';return false;}for(const m of d.members){let id=String(m.user_id);if(id===ME)continue;let offerer=ME<id;await makePeer(id,m.name,offerer);}return true;}catch(e){state.textContent=e.message;return false;}}
async function poll(){try{let d=await api('/api/connect/group-call/signals/'+encodeURIComponent(callId)+(lastSignal?'?since='+encodeURIComponent(lastSignal):''));for(const s of d.signals){await process(s);lastSignal=s.created_at||lastSignal;}}catch(e){} }
async function start(){try{await api('/api/connect/group-call/join/'+encodeURIComponent(callId),{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});localStream=await navigator.mediaDevices.getUserMedia({audio:true,video:true});addTile(ME,'You',localStream,true);await refresh();state.textContent='🟢 Group video is live';setInterval(async()=>{if(await refresh())await poll();},1200);await poll();}catch(e){state.textContent='Could not join group video: '+e.message;}}
document.getElementById('mute').onclick=()=>{let t=localStream&&localStream.getAudioTracks()[0];if(t){t.enabled=!t.enabled;document.getElementById('mute').textContent=t.enabled?'🎙️ Mute':'🔇 Unmute';}};document.getElementById('camera').onclick=()=>{let t=localStream&&localStream.getVideoTracks()[0];if(t){t.enabled=!t.enabled;document.getElementById('camera').textContent=t.enabled?'📷 Camera':'🚫 Camera';}};document.getElementById('hang').onclick=async()=>{try{await api('/api/connect/group-call/leave/'+encodeURIComponent(callId),{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});}catch(e){}if(localStream)localStream.getTracks().forEach(t=>t.stop());Object.values(peers).forEach(x=>x.pc.close());location.href='/connect/chat/'+encodeURIComponent({{ call.conversation_id|tojson }});};window.addEventListener('pagehide',()=>{navigator.sendBeacon&&navigator.sendBeacon('/api/connect/group-call/leave/'+encodeURIComponent(callId),new Blob([JSON.stringify({})],{type:'application/json'}));});start();</script>''',call=call,c=c,uid=uid)
