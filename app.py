...
def research_web(query, limit=8):
    ...
    # Research-only AI configuration. Never expose the secret itself.
    # Render normally provides OPENAI_API_KEY as an environment variable.
    api_key = (os.environ.get("OPENAI_API_KEY") or os.environ.get("AI_API_KEY") or "").strip()
    if not api_key:
        logger.warning("Research AI unavailable: OPENAI_API_KEY/AI_API_KEY is not visible to the running KOJA process.")
        return []

    endpoint = (os.environ.get("AI_API_URL") or "https://api.openai.com/v1/responses").strip()
    model = (os.environ.get("RESEARCH_WEB_MODEL") or os.environ.get("AI_MODEL") or "gpt-5.6-luna").strip()
    payload = {
        "model": model,
...
def research_status():
    """Research-only diagnostic; never returns the API key or its value."""
    key_name = None
    if (os.environ.get("OPENAI_API_KEY") or "").strip():
        key_name = "OPENAI_API_KEY"
    elif (os.environ.get("AI_API_KEY") or "").strip():
        key_name = "AI_API_KEY"
    return {
        "research_ai_configured": bool(key_name),
