...
</style></head><body>{{ screen_body|safe }}</body></html>
"""
...
<section class="media-feed">{% for p in posts %}<article class="media-post" id="post-{{ p.id }}">{% if (p.media_type or '') == 'video' or (p.media_url|lower).endswith(('.mp4','.webm','.mov')) %}<video class="media-post-media video" controls playsinline preload="metadata" src="{{ p.browser_media_url or p.media_url }}" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"></video><div class="media-error" style="display:none;min-height:50vh;place-items:center;background:#000;color:#9fb0bf;text-align:center;padding:30px">Video could not be loaded. Check the media file or storage URL.</div>{% else %}<img class="media-post-media" src="{{ p.browser_media_url or p.media_url }}" alt="{{ p.title or 'KOJA Media' }}" loading="eager" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><div class="media-error" style="display:none;min-height:50vh;place-items:center;background:#000;color:#9fb0bf;text-align:center;padding:30px">Image could not be loaded. Check the media file or storage URL.</div>{% endif %}<div class="media-overlay"><div class="media-meta"><div class="avatar">{{ (p.author_name or 'K')[:1]|upper }}</div><div><strong>{{ p.author_name }}</strong><div style="font-size:10px;color:#aab8c5">{{ p.created_at }}</div></div></div>{% if p.title %}<div class="media-title">{{ p.title }}</div>{% endif %}<div class="media-body">{{ p.body }}</div><div class="media-actions">{% if user %}<form method="post" action="{{ url_for('public_toggle_like',post_id=p.id,next='/public') }}"><button type="submit">{{ '❤️ Liked' if p.liked else '🤍 Like' }} · {{ p.like_count }}</button></form>{% else %}<a href="{{ url_for('login',next='/public') }}">🤍 Like · {{ p.like_count }}</a>{% endif %}<a href="#comments-{{ p.id }}">💬 {{ p.comments|length }}</a>{% if user and p.author_id and p.author_id|string != user.id|string %}<a class="call" href="{{ url_for('connect_call',user_id=p.author_id,mode='video') }}">🎥 Live Call</a>{% endif %}<button type="button" onclick="navigator.clipboard&&navigator.clipboard.writeText(location.origin+'/public#post-{{ p.id }}').then(()=>this.textContent='✓ Shared')">↗ Share</button></div></div><div class="media-comments" id="comments-{{ p.id }}">{% for c in p.comments %}<div><strong>{{ c.author_name }}</strong> — {{ c.body }}<br><span style="color:#7e8d9b">{{ c.created_at }}</span></div>{% endfor %}{% if user %}<form method="post" action="{{ url_for('public_comment',post_id=p.id,next='/public') }}"><input name="body" maxlength="1000" placeholder="Comment…" required><button type="submit">Send</button></form>{% endif %}</div></article>{% else %}<div class="empty">No media published yet.</div>{% endfor %}</section>
</div></div>
