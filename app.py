...
<input id="delivery_id" placeholder="Paste delivery UUID if this is an assigned delivery">
<div class="actions">
...
<div class="hero"><h2>Delivery Tracking</h2><p>Tracking code: <strong>{{ delivery.get("tracking_code") }}</strong></p></div>
<div class="card">
<p><strong>Pickup:</strong> {{ delivery.get("pickup_location") }}</p>
<p><strong>Destination:</strong> {{ delivery.get("destination") }}</p>
<p><strong>Status:</strong> <span id="delivery-status">{{ delivery.get("status") }}</span></p>
<div id="map"></div><p id="tracking-status" class="small">Waiting for driver's location...</p>
</div>
<script>
const trackingCode={{ delivery.get("tracking_code")|tojson }};
let map=L.map("map").setView([-13.9626,28.3228],6),marker=null;
L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:19,attribution:"&copy; OpenStreetMap contributors"}).addTo(map);
async function load(){
 try{
  ...
  if(!d.ok){document.getElementById("tracking-status").textContent=d.message||"No driver GPS available.";return}
  const p=[d.latitude,d.longitude];
  if(!marker){marker=L.marker(p).addTo(map).bindPopup("Driver location");map.setView(p,15)}else marker.setLatLng(p);
  document.getElementById("tracking-status").textContent="Driver location updated: "+(d.updated_at||"recently");
 }catch(e){document.getElementById("tracking-status").textContent="Unable to obtain driver location."}
}
...
def google_search_console_report(days=28, dimension="query"):
    """Return Search Console performance data when optional OAuth credentials are configured."""
    result = {"connected": False, "message": "Google Search Console API is not configured.", "rows": []}
    ...
    try:
        ...
        session_http = AuthorizedSession(credentials)
        ...
        if not response.ok:
            result["message"] = f"Google API returned HTTP {response.status_code}: {response.text[:400]}"
            return result
        data = response.json()
        rows = []
        ...
        result["message"] = "Google Search Console credentials are configured, but google-auth is not installed on Render. Add google-auth to requirements.txt."
        return result
    except Exception as exc:
        logger.exception("Google Search Console error: %s", exc)
        result["message"] = f"Search Console connection error: {str(exc)[:500]}"
        return result
...
@app.route("/admin/search-distribution")
@admin_required
def admin_search_distribution():
    days_raw = request.args.get("days", "28")
    try:
    ...
    indexed_candidates = [u for u in PUBLIC_INDEX_ROUTES]
    return render_page("Google Search & Distribution", r"""
<div class="hero">
  <h2>🔎 Google Search & Distribution</h2>
  <p>Monitor KOJA AFRICA's Google Search visibility, discoverability and distribution from the administrator dashboard.</p>
  <div class="actions">
...
<div class="grid">
  <div class="stat"><div class="big">{{ 'CONNECTED' if query_report.connected else 'SETUP' }}</div>Search Console API</div>
  <div class="stat"><div class="big">{{ indexed_candidates|length }}</div>Public sitemap URLs</div>
  <div class="stat"><div class="big">{{ query_report.rows|length if query_report.connected else '—' }}</div>Search rows</div>
  <div class="stat"><div class="big">{{ days }}</div>Reporting days</div>
</div>

<div class="card">
  <h3>Search Console connection</h3>
  <p>{{ query_report.message }}</p>
  {% if not query_report.connected %}
  <p class="small">To show real Google clicks, impressions, CTR and average position inside KOJA, create a Google Cloud service account, grant it access to the KOJA Search Console property, and set <code>GSC_SERVICE_ACCOUNT_JSON</code> and <code>GSC_SITE_URL</code> in Render.</p>
  {% else %}
  <p class="small">Reporting period: {{ query_report.start }} to {{ query_report.end }}. Google Search Console data can be delayed and is not the same as a live crawler counter.</p>
  {% endif %}
</div>

{% if query_report.connected %}
<div class="card">
  <div class="actions"><h3 style="margin-right:auto">Top Google Search Queries</h3><a class="btn secondary" href="{{ url_for('admin_search_distribution',days=7) }}">7 days</a><a class="btn secondary" href="{{ url_for('admin_search_distribution',days=28) }}">28 days</a><a class="btn secondary" href="{{ url_for('admin_search_distribution',days=90) }}">90 days</a></div>
  <table><tr><th>Query</th><th>Clicks</th><th>Impressions</th><th>CTR</th><th>Avg. position</th></tr>
  {% for r in query_report.rows %}<tr><td>{{ r.key }}</td><td>{{ r.clicks }}</td><td>{{ r.impressions }}</td><td>{{ r.ctr }}%</td><td>{{ r.position }}</td></tr>{% else %}<tr><td colspan="5">No Search Console rows are available for this period.</td></tr>{% endfor %}</table>
</div>
<div class="card">
  <h3>Top Pages from Google Search</h3>
  <table><tr><th>Page</th><th>Clicks</th><th>Impressions</th><th>CTR</th><th>Avg. position</th></tr>
  {% for r in page_report.rows %}<tr><td>{{ r.key }}</td><td>{{ r.clicks }}</td><td>{{ r.impressions }}</td><td>{{ r.ctr }}%</td><td>{{ r.position }}</td></tr>{% else %}<tr><td colspan="5">No page data is available for this period.</td></tr>{% endfor %}</table>
</div>
{% endif %}
...
<div class="card">
  ...
  <div class="grid">
    <div class="card"><strong>✓ robots.txt</strong><p class="small">Allows public crawling while keeping admin, authentication and API routes out of the crawl path.</p></div>
    <div class="card"><strong>✓ XML sitemap</strong><p class="small">Publishes the main public KOJA URLs for Google discovery.</p></div>
    <div class="card"><strong>✓ Search Console</strong><p class="small">Use the official Google property to inspect URLs, submit the sitemap and monitor indexing.</p></div>
    <div class="card"><strong>✓ Performance API</strong><p class="small">With OAuth configured, KOJA can display real Search Console clicks, impressions, CTR and average position.</p></div>
  </div>
</div>

<div class="card">
  <h3>Public URLs being distributed</h3>
  <table><tr><th>URL</th><th>Purpose</th></tr>
  {% for u in indexed_candidates %}<tr><td><a href="{{ SITE_URL }}{{ u }}" target="_blank">{{ SITE_URL }}{{ u }}</a></td><td>Public KOJA page</td></tr>{% endfor %}</table>
</div>
""", query_report=query_report, page_report=page_report, days=days, indexed_candidates=indexed_candidates, SITE_URL=SITE_URL)
...
<div class="card"><h2>Deliveries</h2><table><tr><th>Tracking</th><th>Customer</th><th>Pickup</th><th>Destination</th><th>Driver</th><th>Status</th></tr>
{% for d in rows %}<tr><td>{{ d.get("tracking_code") }}</td><td>{{ d.get("customer_id") }}</td><td>{{ d.get("pickup_location") }}</td><td>{{ d.get("destination") }}</td><td>{{ d.get("driver_id") or "Unassigned" }}</td><td>{{ d.get("status") }}</td></tr>{% endfor %}
</table></div>
""",rows=rows)
