...
<style>.call-icon-btn{width:44px;height:44px;border:0;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:20px;cursor:pointer;margin:0 5px;box-shadow:0 2px 8px rgba(0,0,0,.15);transition:transform .15s ease,opacity .15s ease}.call-icon-btn:hover{transform:scale(1.06)}.call-icon-btn:active{transform:scale(.94)}.call-icon-btn:disabled{opacity:.55;cursor:wait}.call-icon-btn.voice{background:#e8f5e9}.call-icon-btn.video{background:#e3f2fd}</style><div class="card" id="messages" style="min-height:300px;max-height:55vh;overflow:auto"></div><div class="card"><form id="sendForm" class="actions"><input id="text" autocomplete="off" maxlength="4000" placeholder="Write a message…" style="flex:1"><button id="sendBtn">Send</button></form><div class="grid"><label class="btn secondary" style="cursor:pointer">📎 File<input id="filePick" type="file" hidden></label><button type="button" id="voiceNote">🎙️ Voice message</button>{% if other_id %}<button type="button" class="call-icon-btn voice" id="voiceCallBtn" data-call-url="{{ url_for('connect_call',user_id=other_id,mode='voice') }}" aria-label="Voice Call" title="Voice Call">📞</button><button type="button" class="call-icon-btn video" id="videoCallBtn" data-call-url="{{ url_for('connect_call',user_id=other_id,mode='video') }}" aria-label="Video Call" title="Video Call">🎥</button>{% else %}<span class="small">Call unavailable until the other participant is identified.</span>{% endif %}</div><div id="uploadState" class="small"></div></div><script>const cid={{ conversation_id|tojson }};const box=document.getElementById('messages'),text=document.getElementById('text'),sendBtn=document.getElementById('sendBtn');let lastCount=0;function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}function render(m){let content=m.message_type==='text'?esc(m.body):'<a target="_blank" rel="noopener" href="'+esc(m.file_url||'#')+'">📎 '+esc(m.body||m.message_type)+'</a>';return '<div class="card"><strong>'+esc(m.sender_name)+'</strong><div>'+content+'</div><div class="small">'+esc(m.created_at||'')+'</div></div>';}async function load(force=false){try{let r=await fetch('/api/connect/messages/'+cid,{cache:'no-store'});if(!r.ok)return;let d=await r.json();if(force||d.messages.length!==lastCount){box.innerHTML=d.messages.map(render).join('');box.scrollTop=box.scrollHeight;lastCount=d.messages.length;}}catch(e){}}
...
async function startCall(btn,label){if(!btn||!btn.dataset.callUrl){alert('The other participant could not be identified. Please reopen this chat.');return;}let original=btn.textContent;btn.disabled=true;btn.textContent='Connecting…';try{let target=btn.dataset.callUrl;let r=await fetch(target,{method:'GET',redirect:'follow',cache:'no-store',credentials:'same-origin'});if(!r.ok){let t=await r.text().catch(()=> '');console.error('KOJA call start failed',r.status,t.slice(0,500));alert('Could not start the '+label+' call. Server response: '+r.status);btn.disabled=false;btn.textContent=original;return;}window.location.href=target;}catch(e){console.error('KOJA call start error',e);alert('Could not start the '+label+' call. Check your connection and try again.');btn.disabled=false;btn.textContent=original;}}const vb=document.getElementById('voiceCallBtn'),vidb=document.getElementById('videoCallBtn');if(vb)vb.onclick=()=>startCall(vb,'voice');if(vidb)vidb.onclick=()=>startCall(vidb,'video');
document.getElementById('sendForm').onsubmit=async e=>{e.preventDefault();let v=text.value.trim();if(!v)return;sendBtn.disabled=true;try{let r=await fetch('/api/connect/messages/'+cid,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:v})});if(r.ok){text.value='';await load(true);}}finally{sendBtn.disabled=false;text.focus();}};document.getElementById('filePick').onchange=async e=>{let f=e.target.files[0];if(!f)return;if(f.size>15*1024*1024){alert('File must be 15 MB or smaller.');e.target.value='';return;}let st=document.getElementById('uploadState');st.textContent='Uploading '+f.name+'…';let fd=new FormData();fd.append('file',f);try{let r=await fetch('/api/connect/messages/'+cid+'/upload',{method:'POST',body:fd});st.textContent=r.ok?'File sent.':'Upload failed.';await load(true);}catch(x){st.textContent='Upload failed.';}e.target.value='';};let rec,parts=[];document.getElementById('voiceNote').onclick=async()=>{try{let st=await navigator.mediaDevices.getUserMedia({audio:true});rec=new MediaRecorder(st);parts=[];rec.ondataavailable=e=>parts.push(e.data);rec.onstop=async()=>{let b=new Blob(parts,{type:'audio/webm'}),fd=new FormData();fd.append('file',b,'voice.webm');await fetch('/api/connect/messages/'+cid+'/upload',{method:'POST',body:fd});st.getTracks().forEach(t=>t.stop());load(true);};rec.start();setTimeout(()=>rec&&rec.state==='recording'&&rec.stop(),60000);}catch(e){alert('Microphone permission is required.');}};load(true);checkIncoming();setInterval(load,1500);setInterval(checkIncoming,2000);</script>''',conversation_id=conversation_id,name=_profile_name(other_id) if other_id else c.get('name','KOJA Chat'),other_id=other_id)
...
def connect_call(user_id):
    ...
    # Create the ringing call server-side so a browser-side JavaScript failure cannot prevent the call from reaching the other user.
    existing=db_select('koja_calls',filters={'caller_id':uid,'callee_id':user_id,'status':'ringing'},order='created_at.desc',limit=1)
    call=existing[0] if existing else None
    if not call:
...
def connect_call_create():
    ...
    existing=db_select('koja_calls',filters={'caller_id':uid,'callee_id':callee,'status':'ringing'},order='created_at.desc',limit=1)
    if existing:return jsonify(call=existing[0])
