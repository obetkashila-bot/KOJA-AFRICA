...
def register():
    if request.method=="POST":
        ...
        if err:
            # Supabase can report an already-registered account with several
            # different messages depending on the Auth configuration. Only
            # redirect to login for a genuine duplicate; never hide other
            # configuration/network/database errors as "email exists".
            emsg=str(err).strip()
            low=emsg.lower()
            duplicate=(
                "already registered" in low or
                "already exists" in low or
                "user already" in low or
                "email_exists" in low or
                "email address is already" in low or
                "user_already_exists" in low
            )
            if duplicate:
                flash("An account with this email already exists. Please log in.")
                return redirect(url_for("login"))
            flash("Registration failed: "+emsg)
            return redirect(url_for("register"))
        data=data or {}
        au=data.get("user") or {}
        if not au.get("id"):
            flash("Registration failed: Supabase did not return a user account.")
            return redirect(url_for("register"))
        row,perr=sync_profile(au,name,phone)
        if perr:
            # Do not silently send the user to login when the Auth account was
            # created but the profile could not be synchronised.
            logging.error("Profile synchronisation failed for %s: %s",email,perr)
            flash("Account was created, but profile setup failed. Please contact KOJA support.")
            return redirect(url_for("login"))
        if data.get("session"):
        ...
        flash("Account created successfully. Check your email to confirm your account, then log in.")
        return redirect(url_for("login"))
