...
APP_VERSION = "2026.09.05-V38-RESEARCH-WEB-FIX"
...
def research_web(query, limit=8):
    """Reliable web research for Render using OpenAI hosted Web Search.

    This replaces the Render-prone DuckDuckGo Instant Answer request while
    preserving the existing KOJA research pipeline and source records.
    """
    q = clean(query)
    if not q:
        return []
    ...
    if not api_key:
        logger.warning("Web research unavailable: AI_API_KEY/OPENAI_API_KEY is not configured.")
        return []
    ...
    payload = {
        ...
        "input": (
            "Research this topic using current public web sources. "
            "Use reliable sources and do not invent citations. "
            "Topic: " + q
        ),
    ...
    try:
        r = requests.post(
            endpoint, json=payload, timeout=35,
