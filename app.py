...
<div class="card" style="margin-top:10px"><label><input id="attachLocation" type="checkbox" style="width:auto"> 📍 Attach my current location</label><input type="hidden" name="latitude" id="postLat"><input type="hidden" name="longitude" id="postLon"><input name="location_label" id="postLocationLabel" placeholder="Location name (optional)"><button class="btn secondary" type="button" onclick="getPostLocation()">Use GPS Location</button><span id="postGpsStatus" class="small"></span></div>
<button class="btn" type="submit">🌐 Publish Publicly</button></form>
<script>function getPostLocation(){const s=document.getElementById('postGpsStatus');if(!navigator.geolocation){s.textContent='GPS is not supported on this device.';return}s.textContent='Getting location…';navigator.geolocation.getCurrentPosition(p=>{document.getElementById('postLat').value=p.coords.latitude.toFixed(7);document.getElementById('postLon').value=p.coords.longitude.toFixed(7);document.getElementById('attachLocation').checked=true;s.textContent='Location captured. It will be attached when you publish.'},e=>{s.textContent='Location permission was not granted.'},{enableHighAccuracy:true,timeout:12000,maximumAge:30000})}</script>
...
{% if p.media_url %}<img src="{{ p.media_url }}" alt="Public KOJA post image" loading="lazy" style="width:100%;max-height:620px;object-fit:cover;border-radius:12px;margin-top:8px">{% endif %}{% if p.latitude and p.longitude %}<p class="small">📍 {{ p.location_label or 'Shared location' }} · <a target="_blank" rel="noopener" href="https://www.openstreetmap.org/?mlat={{ p.latitude }}&mlon={{ p.longitude }}#map=16/{{ p.latitude }}/{{ p.longitude }}">View map</a></p>{% endif %}
