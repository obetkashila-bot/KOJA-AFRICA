...
APP_VERSION = "2026.09.08-FLW-V3-ZM-MM-FIX1"
...
def enforce_csrf():
    ...
    if request.path in ("/api/professional/presence", "/webhook/flutterwave"):
        return None
...
def marketplace_buy(product_id):
    ...
    if err or not order:
        ...
        return redirect(url_for('marketplace_product_view',product_id=product_id))
    network=clean(request.form.get('network') or user.get('mobile_money_network') or 'MTN').upper()
    if network not in ('MTN','AIRTEL','ZAMTEL'):
        network='MTN'
    phone=clean(request.form.get('phone_number') or user.get('phone'))
    if not phone:
        flash('Please add your mobile-money phone number before payment can start.','warning')
        return redirect(url_for('marketplace_product_view',product_id=product_id))
    payload={'phone_number':phone,'network':network,'amount':amount,'currency':product.get('currency') or 'ZMW','fullname':first_nonempty(user.get('name'),email),'email':email,'tx_ref':tx_ref,'order_id':str(order.get('id') or tx_ref),'meta':{'koja_order_id':str(order.get('id') or ''),'product_id':str(product_id),'buyer_id':str(uid or '')}}
    try:
        r=requests.post(FLW_BASE_URL+'/charges?type=mobile_money_zambia',headers={'Authorization':'Bearer '+FLW_SECRET_KEY,'Content-Type':'application/json'},json=payload,timeout=30)
        data=json_or_empty(r)
        auth=((data.get('meta') or {}).get('authorization') or {}) if isinstance(data,dict) else {}
        link=auth.get('redirect') or ((data.get('data') or {}).get('link') if isinstance(data,dict) else None)
        if r.ok and link:
            return redirect(link)
        if r.ok and isinstance(data,dict) and data.get('status') == 'success':
            flash('Payment request sent. Please authorize the mobile-money payment on your phone.','success')
            return redirect(url_for('marketplace_my'))
        logger.error('Flutterwave Zambia mobile-money charge failed: %s %s',r.status_code,str(data)[:2000])
    except Exception as exc:
        logger.exception('Flutterwave Zambia mobile-money charge error: %s',exc)
    flash('Flutterwave mobile-money payment could not be started. Please check your phone number, network and try again.','danger')
    return redirect(url_for('marketplace_product_view',product_id=product_id))
...
def flutterwave_webhook():
    ...
    received=clean(request.headers.get('verif-hash'))
    if not secret_hash or not received or not secrets.compare_digest(received, secret_hash):
        return jsonify({'ok':False}), 401
    body=request.get_json(silent=True) or {}
    data=body.get('data') or {}
    tx_ref=clean(data.get('tx_ref'))
    transaction_id=clean(data.get('id'))
    status=clean(data.get('status')).lower()
    if not tx_ref or not transaction_id:
        return jsonify({'ok':True,'ignored':True}), 200
    order=first_row('koja_marketplace_orders',{'payment_reference':tx_ref})
    if not order:
        return jsonify({'ok':True,'ignored':True}), 200
    # Re-query Flutterwave before releasing the product.
    try:
        r=requests.get(FLW_BASE_URL+'/transactions/'+transaction_id+'/verify',headers={'Authorization':'Bearer '+FLW_SECRET_KEY,'Content-Type':'application/json'},timeout=30)
        verified=(json_or_empty(r).get('data') or {}) if r.ok else {}
        expected_amount=float(order.get('amount') or 0); paid_amount=float(verified.get('amount') or 0)
        valid=(r.ok and verified.get('status')=='successful' and str(verified.get('tx_ref'))==tx_ref and str(verified.get('currency') or '').upper()==str(order.get('currency') or 'ZMW').upper() and paid_amount>=expected_amount)
        if valid:
            db_update('koja_marketplace_orders',{'id':order.get('id')},{'status':'paid','payment_method':'flutterwave','payment_reference':tx_ref,'payment_transaction_id':str(verified.get('id') or transaction_id),'updated_at':utc_now()})
        elif status in ('failed','cancelled'):
            db_update('koja_marketplace_orders',{'id':order.get('id')},{'status':'failed','updated_at':utc_now()})
    except Exception:
        logger.exception('Flutterwave webhook verification failed for %s',tx_ref)
