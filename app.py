...
def connect_chat(conversation_id):
    ...
    return render_page("KOJA Chat",r"""<div class="card"><a href="{{ url_for('connect') }}">← Connect</a><h2>💬 {{ name }}</h2><p class="small">🔒 End-to-end encrypted chat. The server stores ciphertext, not plaintext.</p><div id="e2eeStatus" class="small">Setting up encryption…</div></div><div class="card" id="messages" style="min-height:300px;max-height:55vh;overflow:auto"></div><div class="card"><form id="sendForm"><input id="text" autocomplete="off" placeholder="Write an encrypted message…"><button>Send</button></form><div class="grid"><a class="btn" href="{{ url_for('connect_call',user_id=other_id,mode='voice') }}">📞 Voice Call</a><a class="btn" href="{{ url_for('connect_call',user_id=other_id,mode='video') }}">🎥 Video Call</a>{% if conversation_type=='group' %}<a class="btn" href="{{ url_for('connect_group_call',conversation_id=conversation_id,mode='video') }}">👥 Group Video</a><a class="btn secondary" href="{{ url_for('connect_group_call',conversation_id=conversation_id,mode='voice') }}">👥 Group Voice</a>{% endif %}</div></div><script>
const cid={{ conversation_id|tojson }},me={{ user.id|tojson }};const box=document.getElementById('messages'),text=document.getElementById('text'),statusEl=document.getElementById('e2eeStatus');const te=new TextEncoder(),td=new TextDecoder();
...
let key=null;async function openE2EE(url,meta){try{let x=await decFile(key,url,meta),u=URL.createObjectURL(x.blob),a=document.createElement('a');a.href=u;a.download=x.meta.name||'koja-file';a.target='_blank';a.click();setTimeout(()=>URL.revokeObjectURL(u),60000);}catch(e){alert('Unable to decrypt this attachment on this device.');}}async function load(){let r=await fetch('/api/connect/messages/'+cid);if(!r.ok)return;let d=await r.json(),html='';for(const m of d.messages){let mine=String(m.sender_id)===String(me),body='';if(m.message_type==='text')body='<div>'+esc(await dec(key,m.body))+'</div>';else if(m.message_type==='e2ee_file'&&m.file_url){let label='Encrypted attachment';try{let x=JSON.parse(await dec(key,m.body));label='🔒 '+esc(x.name);}catch(e){}body='<div>'+label+' <button type="button" onclick="openE2EE(\''+esc(m.file_url)+'\',\''+esc(m.body)+'\')">Open</button></div>';}else body='<div>🔒 Encrypted attachment</div>';html+='<div style="display:flex;justify-content:'+(mine?'flex-end':'flex-start')+';margin:7px 0"><div style="max-width:78%;padding:10px 13px;border-radius:16px;background:var(--card);border:1px solid var(--border);text-align:left"><strong>'+esc(mine?'You':m.sender_name)+'</strong>'+body+'<div class="small">'+esc(m.created_at||'')+'</div></div></div>';}box.innerHTML=html;box.scrollTop=box.scrollHeight;}
...
document.getElementById('fileForm').onsubmit=async e=>{e.preventDefault();let f=document.getElementById('file').files[0];if(!f||!key)return;try{let x=await encFile(key,f),fd=new FormData();fd.append('file',x.blob,'e2ee.bin');fd.append('e2ee','1');fd.append('e2ee_meta',x.meta);let r=await fetch('/api/connect/messages/'+cid+'/upload',{method:'POST',body:fd});if(!r.ok)throw Error('Upload failed');document.getElementById('file').value='';load();}catch(err){alert('Encrypted file could not be sent.');}};
...
@app.route('/media')
def media_screen():
    posts=db_select('koja_public_posts',filters={'is_published':'eq.true'},order='created_at.desc',limit=50)
